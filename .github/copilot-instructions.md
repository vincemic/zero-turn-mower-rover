...
- Manual control + on-handset telemetry: **FrSky X8R receiver** (ACCST D16, SBUS output) wired to Cube Orange RCIN, with **FrSky S.Port passthrough telemetry** (`SERIAL2_PROTOCOL=10`, 57600 baud) back to the operator's **FrSky Taranis X9D Plus** (OpenTX 2.3V0026, Yaapu v1.8.0 telemetry script). RC channel mapping confirmed: `RC_PROTOCOLS=1` (SBUS), `MODE_CH=9` (SA 3-pos → Manual/Acro/Auto), `RC7_OPTION=153` (SF 2-pos → arm/disarm). The receiver is for **manual override and at-handset monitoring**, not for replacing GCS/MAVLink mission control. Autonomous mowing still runs from a saved mission via MAVLink; the operator can take manual control at any time via the SA mode switch on the Taranis X9D Plus. Taranis config is version-controlled in `config/taranis/` (EdgeTX YAML export).
...
- Engine/blade I/O: three RC-passthrough relay outputs on Cube Orange MAIN: `SERVO6` (function `58` = RCIn8 → SB switch, starter relay, momentary), `SERVO7` (function `56` = RCIn6 → SC switch, blade clutch), `SERVO8` (function `55` = RCIn5 → SG switch, ignition kill, SF-arm-gated at the transmitter). `SERVO5` (function `0`) is currently disabled / reserved. All AUX pins (`SERVO9`–`SERVO14`) remain free.
...
- Companion: **NVIDIA Jetson AGX Orin Developer Kit (64 GB LPDDR5, 2048 CUDA cores / 16 SMs, 64 Tensor Cores, 275 TOPS, 204.8 GB/s memory bandwidth, JetPack 6 / L4T 36.5, aarch64)** running at **nvpmodel mode 3 (50 W)** with kiosk display + **Luxonis OAK-D Pro** over USB via DepthAI. The OAK-D Pro and Pixhawk are connected through a **Waveshare 4-Ch USB 3.2 Gen1 powered hub** (VIA Labs `2109:0817`). The OAK-D Pro boots in USB 2.0 mode (Movidius MyriadX bootloader at vendor `03e7:2485`); DepthAI uploads firmware and the device re-enumerates as `03e7:f63b` at **USB 3.x SuperSpeed (5 Gbps)** — confirmed working 2026-04-27. **Required setup:** (1) udev rules for non-root access (`80-oakd-usb.rules`, `MODE="0666"` for vendor `03e7`), (2) kernel params `usbcore.autosuspend=-1`, `usbcore.usbfs_memory_mb=1000`, and `usbcore.quirks=03e7:2485:gk,03e7:f63b:gk` (NO_LPM for both bootloader and booted PIDs) in `extlinux.conf`, (3) `depthai>=3.5.0` (v3 API — `Device()` takes no pipeline arg, `getMxId()` deprecated in favor of `getDeviceId()`). All managed by `jetson-harden.sh` and `pyproject.toml` `[jetson]` extras. Default VSLAM config: 800p stereo @ 30 FPS, IR dot projector 750 mA, IR flood LED 200 mA, USB 3.x SuperSpeed — configurable via `/etc/mower/vslam.yaml`.
- Kiosk display: **Weston 13.0.0** (NVIDIA-patched, `nvidia-l4t-weston` package) as Wayland compositor on DRM `card0` (DP-1 output, `nv_platform` driver) with **`--renderer=pixman`** (CPU compositing). **Do not attempt EGL/GL rendering on card0** — on Tegra234, the display controller (`card0`) uses the `nvidia-drm` GBM backend but NVIDIA's EGL external platform (`libnvidia-egl-gbm.so`) only supports the `tegra` GBM backend on `card1`/`host1x`. `eglInitialize()` always fails with `EGL_NOT_INITIALIZED (0x3001)` on card0. The kiosk is a **split-process architecture**: a Python data service (`mower-kiosk-data.service`, runs `mower-jetson kiosk run`) publishes JSON telemetry frames over a Unix socket (`/run/mower/kiosk-display.sock`), and a **native C/LVGL 9.5 renderer** (`mower-kiosk-renderer.service`, binary at `/usr/local/bin/mower-kiosk-renderer`) reads those frames and renders an 8-card operational dashboard to Wayland via LVGL's SHM driver (no EGL/GL). LVGL uses NEON-accelerated software rendering (`LV_DRAW_SW_ASM_NEON`) at 32-bit color depth, ~30 FPS. The renderer reports `sd_notify(READY=1)` and sends periodic `WATCHDOG=1`. The fullscreen dashboard must not exceed 1920×1080 or Weston rejects the xdg_surface with a protocol error. Source is in `contrib/lvgl_kiosk/` (CMake, FetchContent for LVGL v9.5.0, vendored cJSON). Managed via `mower-weston.service` + `mower-kiosk-data.service` + `mower-kiosk-renderer.service` (systemd, `seatd` seat backend). Kernel params include `nvidia_drm.modeset=1` in `extlinux.conf`.
- Wireless: **AzureWave AW-CB375NF** (Intel Wi-Fi 6E AX210) in the Orin Dev Kit M.2 Key E 2230 slot — Wi-Fi 802.11ax tri-band (2.4 / 5 / 6 GHz, 2×2 MIMO, ~2.4 Gbps) + **Bluetooth 5.3**. Pre-installed (not factory default). Linux drivers: `iwlwifi` (Wi-Fi), `btusb` (BT). The hardening script currently disables the `bluetooth` systemd service — re-enable if BT is needed for future features.
...
- **RC is present for manual override and on-handset telemetry**, not as the primary control path. ArduPilot's RC stack is **enabled** (`RC_PROTOCOLS=1` = SBUS, `FS_THR_ENABLE=1`, `FS_THR_VALUE=910`). FrSky S.Port passthrough telemetry runs on `SERIAL2_PROTOCOL=10` at 57600 baud. The mode switch is `MODE_CH=9` (SA 3-pos: Manual/Acro/Auto). Arm/disarm is `RC7_OPTION=153` (SF 2-pos). The autonomous mowing mission still runs from MAVLink/GCS; the Taranis X9D Plus is for the operator to (a) take manual control from any range the handset can reach and (b) read live status without needing the laptop. **The physical E-stop still has absolute authority over both RC and GCS.**
- **Default mower failsafe = Hold, not RTL.** RTL drives in a straight line through obstacles. Target: `FENCE_ACTION=2`, `FS_EKF_ACTION=2`. **Current state (2026-05-01):** `FENCE_ACTION=1` (RTL), `FS_EKF_ACTION=1` (RTL), `FENCE_ENABLE=0` — these need correction before autonomous field testing.
...
- Where vision and research disagree, **research wins** (notably: GNSS receiver, ignition relay, wheel encoders). The 2026-05-01 param dump (`docs/config/mower.param`) is the authoritative record of the Pixhawk's current state.
