...
- Kiosk display: **Weston 13.0.0** (NVIDIA-patched, `nvidia-l4t-weston` package) as Wayland compositor on DRM `card0` (DP-1 output, `nv_platform` driver) with **`--renderer=pixman`** (CPU compositing). **Do not attempt EGL/GL rendering on card0** — on Tegra234, the display controller (`card0`) uses the `nvidia-drm` GBM backend but NVIDIA's EGL external platform (`libnvidia-egl-gbm.so`) only supports the `tegra` GBM backend on `card1`/`host1x`. `eglInitialize()` always fails with `EGL_NOT_INITIALIZED (0x3001)` on card0. The kiosk client is a **native GTK4 app** (`mower-jetson kiosk run`) using the Cairo GSK renderer — `GSK_RENDERER=cairo` is **mandatory** in the kiosk unit environment (GTK4's default GL renderer fails on pixman Weston). GTK4 also requires `Gio.ApplicationFlags.NON_UNIQUE` because there is no D-Bus session bus (without it, `activate` never fires). The fullscreen dashboard grid must not exceed 1920px natural width or Weston rejects the xdg_surface with a protocol error. Managed via `mower-weston.service` + `mower-kiosk.service` (systemd, `seatd` seat backend). Kernel params include `nvidia_drm.modeset=1` in `extlinux.conf`.
