...
def check_fence_match(
    ...
    if len(expected_items) != len(fence_items):
        detail = (
            f"Fence vertex count mismatch: expected"
            f" {len(expected_items)}, got {len(fence_items)}"
        )
        log.warning(
            "fence count mismatch",
            expected=len(expected_items),
            actual=len(fence_items),
        )
        return False, detail

    # Compare each vertex
    for i, (expected, actual) in enumerate(zip(expected_items, fence_items, strict=False)):
        ...
        if lat_diff > tolerance or lon_diff > tolerance:
            detail = (
                f"Fence vertex {i} mismatch: expected"
                f" ({expected_lat:.7f}, {expected_lon:.7f}),"
                f" got ({actual_lat:.7f}, {actual_lon:.7f})"
                f" (diff: lat={lat_diff:.2e},"
                f" lon={lon_diff:.2e})"
            )
            log.warning(
                "fence vertex mismatch",
                vertex=i,
                lat_diff=lat_diff,
                lon_diff=lon_diff,
            )
    ...
    detail = (
        f"Fence matches zone boundary ({len(fence_items)} vertices,"
