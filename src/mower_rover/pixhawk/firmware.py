...
from mower_rover.pixhawk._uploader import firmware as ApjFirmware, uploader, INSYNC, EOC, OK, GET_SYNC
...
def check_remote_version(track: str = "stable") -> tuple[int, int, int] | None:
    ...
    try:
        ...
        version_url = f"{FIRMWARE_BASE_URL.replace('/stable/', f'/{url_path}/')}/firmware-version.txt"
...
def validate_apj(apj_path: Path) -> None:
    ...
    try:
        ...
        raise ValueError(f"Invalid JSON in APJ file: {e}")
...
def reboot_to_bootloader(conn: Any) -> None:
    ...
    try:
        ...
        raise ConnectionError(f"Failed to reboot to bootloader: {e}")
