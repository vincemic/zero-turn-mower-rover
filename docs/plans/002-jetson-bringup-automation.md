...
[Unit]
Description=Mower Rover Jetson Daemon
After=network-online.target
Wants=network-online.target

[Service]
Type=notify
ExecStart={mower_jetson_path} service run
...
The `{mower_jetson_path}` is resolved via `shutil.which("mower-jetson")` at install time. The unit file is written to `~/.config/systemd/user/mower-jetson.service` (user-level systemd) or `/etc/systemd/system/mower-jetson.service` (system-level, requires sudo).
